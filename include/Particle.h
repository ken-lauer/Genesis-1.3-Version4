...
private:
    ...
    public:
        ...
        // proxy value, good enough for range-for, sized copies and the other
        // read-mostly uses in this codebase. Algorithms that swap through
        // iterators (std::sort et al.) do not compile against it - the proxy
        // has no swap - which is intentional: a copy-constructed proxy rebinds
        // instead of copying the particle, so a generic swap would corrupt data.
        ...
        Iter(const Iter<B> &o) : c_(o.c_), i_(o.i_) {}
        reference operator*() const { return (*c_)[i_]; }
        reference operator[](difference_type d) const
        {
            return (*c_)[i_ + static_cast<std::size_t>(d)];
        }
        ...
        friend Iter operator+(difference_type d, Iter it) { it += d; return it; }
        friend Iter operator-(Iter it, difference_type d) { it -= d; return it; }
...
public:
    ...
    // raw component spans for the SIMD kernels (contiguous, packet-aligned)
...
private:
    // Eigen's allocator aligns to EIGEN_MAX_ALIGN_BYTES = AlignedMax, exactly
    // what the kernels' MapAligned loads assert on every instruction set; the
    // padding keeps every whole-batch offset into the arrays equally aligned
    using dvec = std::vector<double, Eigen::aligned_allocator<double>>;
